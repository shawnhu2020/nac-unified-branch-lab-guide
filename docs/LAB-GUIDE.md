...
This lab introduces **Unified Branch - Network as Code (NaC)** — an approach to managing [**Unified Branch**](https://www.cisco.com/site/us/en/solutions/networking/campus-branch-networking/unified-branch/index.html?dtid=osscdc000283&linkclickid=srch) network infrastructure declaratively using YAML files, Terraform, and CI/CD automation. You will use a real Meraki lab environment with physical MX, MS, and CW devices.
...
![Unified Branch - Network as Code Lab Topo](assets/media/topo.png)
...
In both labs, you will use automation to create the **Unified Branch 1** and **Unified Branch 2** networks inside your existing lab organization, claim the physical devices, and configure them with the NaC data model.
...
!!! info "Information"
    **Network as Code (NaC)** extends Infrastructure as Code (IaC) principles to network configuration and policy management. Instead of clicking through a GUI, you describe your desired network state in YAML files, and automation tools translate those files into Meraki API calls that configure your devices.

    **Branch as Code (BaC)** is the specialized application of NaC to Cisco Unified Branch deployments. It uses Terraform and a curated set of YAML data model files to provision network infrastructure consistently across many branch sites.
    ...
    **Note**: In this lab, we are using a different switch model that is outside the CVD scope. However, the learning experience for Network as Code remains the same.
